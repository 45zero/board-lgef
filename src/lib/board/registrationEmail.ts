...
export type EmailBlock =
  ...
  | { id: string; type: "date" }
  | { id: string; type: "banner"; url: string }
  | { id: string; type: "map"; label: string; address: string }
  | { id: string; type: "video"; url: string }
  | { id: string; type: "pdf"; url: string; filename: string }
  | { id: string; type: "signature"; name: string; title: string; imageUrl: string | null }
  | { id: string; type: "buttons" };
...
function renderBlock(block: EmailBlock, c: RegistrationEmailContent): string {
  switch (block.type) {
    ...
    case "date":
      return `<p style="margin:0 0 16px;color:#0b1d3c;font-size:16px;font-weight:bold;">📅 ${escapeHtml(c.eventDateLabel)}${c.eventLocation ? ` — ${escapeHtml(c.eventLocation)}` : ""}</p>`;

    case "image":
      return block.url
        ? `<img src="${escapeAttr(block.url)}" alt="" style="width:100%;max-width:504px;border-radius:12px;display:block;margin:0 0 20px;" />`
        : "";
    ...
    case "video":
      return block.url
        ? `<a href="${escapeAttr(block.url)}" style="display:block;text-decoration:none;margin:0 0 20px;">
    ...
    case "pdf":
      return block.url
        ? `<a href="${escapeAttr(block.url)}" style="display:flex;align-items:center;gap:8px;background:#f7f9fd;border:1px solid #eaeff7;border-radius:10px;padding:12px 14px;margin:0 0 20px;color:#12305f;font-weight:bold;font-size:13px;text-decoration:none;">
...
/** Email HTML "design" (table-based, compatible clients mail) — blocs rendus dans l'ordre choisi par l'utilisateur. */
export function buildRegistrationEmailHtml(c: RegistrationEmailContent): string {
  ...
  const bodyHtml = blocks.map((b) => renderBlock(b, c)).join("");
